...
export default function StepDataTable({ step, pageSize = 25, visitId = null }: Props) {
  ...
  const fetchPage = useCallback(async () => {
    ...
    try {
      ...
      const arr: Row[] = Array.isArray(raw) ? (raw as unknown as Row[]) : [];
      setRows(arr);

      // ===== lookups: لو محدد في config
      if (cfg.lookups) {
        const next: Record<string, Record<string, string>> = {};

        for (const [colKey, lu] of Object.entries(cfg.lookups)) {
          // ids الفريدة (string أو number)
          ...
          if (!lres.error && Array.isArray(lres.data)) {
            ...
            const list = lres.data as unknown as Record<string, unknown>[];
      ...
      console.error("[StepDataTable] fetch error", e);
      setErrMsg(msg);
      setRows([]);
    } finally {
      setLoading(false);
