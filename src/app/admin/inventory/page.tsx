...
import SupaImg from "@/components/SupaImg"; // <-- الخطوة 1: استيراد المكون الناجح
...
function EmptyBox({ text }: { text: string }) {
  return (
    <div style={{ border: cardBorder, background: "var(--input-bg)", borderRadius: 12, padding: 24, textAlign: "center", fontSize: 13, opacity: 0.85 }}>
      {text}
    </div>
  );
}
...
function btn(h?: number, selected = false): React.CSSProperties {
  return {
    ...
    height: h || 44, padding: "10px 12px", borderRadius: 12,
...
function DateField({ value, onChange, label, locale = "en-GB" }: { label: string, value: string; onChange: (v: string) => void; locale?: string; }) {
  ...
  const openPicker = () => { 
    try { inputRef.current?.showPicker(); } 
      ...
      console.error("showPicker() is not supported, falling back to click:", e);
  ...
  return (
    <div>
      ...
      <div role="button" onClick={openPicker} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--input-border)", background: "var(--card)", cursor: "pointer", height: 46 }}>
          <span style={{ fontWeight: 700 }}>{pretty}</span>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="5" width="18" height="16" rx="3" stroke="currentColor" strokeWidth="1.5" /><path d="M16 3v4M8 3v4M3 10h18" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
      </div>
      <input ref={inputRef} type="date" value={value} onChange={(e) => onChange(e.target.value)} style={{ position: "absolute", opacity: 0, pointerEvents: "none" }} />
    </div>
  );
}
...
export default function InventoryReportPage() {
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      setLoading(true);
      const { data: usersData } = await supabase.from("client_users").select("Users!inner(*)").eq("client_id", cid).eq("is_active", true);
      const { data: marketsData } = await supabase.from("client_markets").select("Markets!inner(*)").eq("client_id", cid);
      setAllClientUsers((usersData?.map(i => i.Users) || []) as UserRow[]);
      setAllClientMarkets((marketsData?.map(i => i.Markets) || []) as MarketRow[]);
      setLoading(false);
    };
    fetchInitialData();
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      ...
      if (error) console.error("Error fetching all reports:", error);
      else setReports((data || []) as InventoryReport[]);
      setLoading(false);
    };
    fetchReports();
  }, [clientId]);
  ...
  const resetFilters = () => {
    setDateFrom(''); setDateTo(''); setSelectedTL('ALL'); setSelectedRegion(''); setSelectedStore('');
    setSelectedUsers([]); setSelectedMarketStore(null); setSelectedBranchId(null);
  };

  const teamLeaders = useMemo(() => allClientUsers.filter(u => u.role?.toLowerCase().replace(/_/g, ' ').includes('team leader')), [allClientUsers]);
  const regionOptions = useMemo(() => Array.from(new Set(allClientMarkets.map(m => m.region).filter(Boolean))).sort((a,b) => a.localeCompare(b, 'ar')), [allClientMarkets]);
  const storeOptions = useMemo(() => Array.from(new Set(allClientMarkets.map(m => m.store).filter(Boolean))).sort((a,b) => a.localeCompare(b, 'ar')), [allClientMarkets]);

  const filteredReports = useMemo(() => {
      return reports.filter(r => {
          const reportDate = new Date(r.created_at);
          const fromOk = !dateFrom || reportDate >= new Date(dateFrom);
          const toOk = !dateTo || reportDate <= new Date(dateTo + 'T23:59:59');
  ...
  const marketsForPanel = useMemo(() => {
    ...
    reportsFilteredByUsers.forEach(r => { if(r.market?.store) marketStores.add(r.market.store); });
  ...
  const branchesForPanel = useMemo(() => {
    ...
    reportsFilteredByUsers.forEach(r => {
        if(r.market?.store === selectedMarketStore && r.market.id && r.market.branch) { branches.set(r.market.id, r.market.branch); }
    });
  ...
  const openModalWithData = (date: string, branchId: UUID) => {
    const dataForModal = filteredReports.filter(r => 
        r.market?.id === branchId && new Date(r.created_at).toISOString().split('T')[0] === date
    );
  ...
  useEffect(() => {
    const handleEsc = (event: KeyboardEvent) => { if (event.key === 'Escape') { setModalOpen(false); setLightboxImage(null); }};
  ...
  return (
    <div style={{ maxWidth: 1400, margin: "0 auto", padding: 16 }}>
      {loading && ( <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }}><div style={{ color: 'var(--text)' }}>{ar ? "جاري التحميل..." : "Loading..."}</div></div> )}
      
      <h1 style={{ textAlign: 'center', marginBottom: 16, fontSize: '2.25em', color: 'var(--accent)' }}>{T.pageTitle}</h1>
      
      <div style={{ position: "sticky", top: 8, zIndex: 20, marginBottom: 16, padding: 10, borderRadius: 16, border: cardBorder, background: "color-mix(in oklab, var(--card) 82%, transparent)", backdropFilter: 'blur(8px)' }}>
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10, alignItems: 'end' }}>
              ...
              <div>
                ...
                <select onChange={(e) => setSelectedTL(e.target.value as UUID | 'ALL')} value={selectedTL || ''} style={{ width: '100%', height: 46, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--input-border)", background: "var(--card)", color: 'var(--text)'}}>
              ...
              <div>
                ...
                <select onChange={(e) => setSelectedRegion(e.target.value)} value={selectedRegion} style={{ width: '100%', height: 46, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--input-border)", background: "var(--card)", color: 'var(--text)'}}>
              ...
              <div>
                ...
                <select onChange={(e) => setSelectedStore(e.target.value)} value={selectedStore} style={{ width: '100%', height: 46, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--input-border)", background: "var(--card)", color: 'var(--text)'}}>
                  <option value="">{ar ? "الكل" : "All"}</option>
        ...
        <Panel title={ar ? "المنسقون (MCH)" : "Coordinators (MCH)"}>
          {usersForPanel.length === 0 ? <EmptyBox text={ar ? "اختر قائد فريق أو لا يوجد منسقون" : "Select a team leader or no coordinators found"} /> : (
            <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))" }}>
              <button onClick={() => {const allIds = usersForPanel.map(u => u.id); setSelectedUsers(allIds.every(id => selectedUsers.includes(id)) ? [] : allIds);}} style={btn(undefined, usersForPanel.length > 0 && selectedUsers.length === usersForPanel.length)}>
                {ar ? "تحديد الكل" : "Select All"}
              </button>
              {usersForPanel.map(user => {
                const isSelected = selectedUsers.includes(user.id);
                return ( <button key={user.id} onClick={() => setSelectedUsers(p => isSelected ? p.filter(id => id !== user.id) : [...p, user.id])} style={btn(48, isSelected)}>
                    <span>{ar ? user.arabic_name || user.name : user.name || user.arabic_name}</span><span style={{opacity: 0.6}}>{isSelected ? "✓" : "＋"}</span>
                  </button>
                )
              })}
            </div>
            ...
            {marketsForPanel.length === 0 ? <EmptyBox text={ar ? "لا توجد أسواق لهؤلاء المستخدمين" : "No stores for these users"} /> : (
              <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))'}}>
                {marketsForPanel.map(store => (
                  <button key={store} onClick={() => {setSelectedMarketStore(store); setSelectedBranchId(null);}} style={btn(48, selectedMarketStore === store)}>
                    <span>{store}</span><span style={{opacity: 0.6}}>{selectedMarketStore === store ? "✓" : "＋"}</span>
                  </button>
                ))}
              </div>
            )}
              ...
              <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))'}}>
                {branchesForPanel.map(b => (
                  <button key={b.id} onClick={() => setSelectedBranchId(b.id)} style={btn(48, selectedBranchId === b.id)}>
                    <span>{b.branch}</span><span style={{opacity: 0.6}}>{selectedBranchId === b.id ? "✓" : "＋"}</span>
                  </button>
                ))}
              </div>
            )}
                    ...
                    <td style={{padding: '8px', textAlign: 'center'}}>
                      {item.photos && item.photos.length > 0 ? (
                        item.photos.map((photo, i) => (
                          <button onClick={() => setLightboxImage(photo)} key={i} style={{color: 'var(--accent)', textDecoration: 'underline', margin: '0 4px', background: 'none', border: 'none', cursor: 'pointer'}}>{ar ? `صورة ${i+1}` : `Img ${i+1}`}</button>
                        ))
                      ) : '-'}
                    </td>
                  </tr>
      ...
      {lightboxImage && (
  <div onClick={() => setLightboxImage(null)} style={{position: 'fixed', inset: 0, zIndex: 101, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px'}}>
      <SupaImg 
        src={lightboxImage} 
        alt="Inventory" 
        style={{maxWidth: '90%', maxHeight: '90%', objectFit: 'contain'}} 
      />
      <button onClick={() => setLightboxImage(null)} style={{position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', color: 'white', fontSize: 32, cursor: 'pointer'}}>&times;</button>
  </div>
)}
    </div>
  );
